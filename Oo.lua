...
function Library:CreateWindow(info)
	...
	function Library.Tabs:CreateHeader(info)
		...
		function Sections:CreateTab(info)
			...
			function Main:CreateToggle(info)
				...
				local function change()
					...
					if Value then
						...
						tw({v = Frame_2, t = 0.15, s = Enum.EasingStyle.Exponential, d = "Out", g = {Position = UDim2.new(0.27, 0,0.5, 0), BackgroundColor3 = Color3.fromRGB(65, 65, 65)}}):Play()
			...
			function Main:Paragarp(info)
				local Title = info.Title
				local Desc = info.Desc or ''
				local Image = info.Image
				local Paragarp = background(Section_1, Title, Desc)
				local New = {}

				function New:SetTitle(a)
					Paragarp.Frame.Title.Text = a
				end

				function New:SetDesc(a)
					if a and a ~= '' then
						Paragarp.Frame.Desc.Text = a
						Paragarp.Frame.Desc.Visible = true
					else
						Paragarp.Frame.Desc.Visible = false
					end
				end
			end
			...
			function Main:Image(info)
				...
				Image_1.Size = UDim2.new(1, 0, 0, Size)
	...
	do
		...
		Size.Position = UDim2.new(1, 0, 1, 0)
		...
		U.InputChanged:Connect(function(i)
			if R and i.UserInputType == Enum.UserInputType.MouseMovement then
				local nW = math.max(420, i.Position.X - Background_1.AbsolutePosition.X)
				local nH = math.max(148, i.Position.Y - Background_1.AbsolutePosition.Y)
